...
const PasskeyRegistration = ({ passkeys = [] }) => {
    const [working, setWorking] = useState(false);
    const [registrationError, setRegistrationError] = useState(null);

    const startWebauthnPasskeyRegistration = async () => {
        try {
            setWorking(true);
            ...
            <section className={working ? 'cursor-wait' : ''}>
                ...
                <div className="mt-4 shadow-sm rounded-lg">
                    <button 
                        type="button" 
                        disabled={working} 
                        onClick={startWebauthnPasskeyRegistration}
                ...
                <div className="mt-6 bg-gray-300 dark:bg-gray-700 shadow-sm rounded-lg divide-y">
                    {passkeys.length > 0 ? (
                                ...
                                <div className="flex justify-between items-center">
                                    <div>
                                        ...
                                        {passkey.last_used_at && (
                                            <small className="ml-2 text-sm text-gray-600 dark:text-gray-300">
                                                {passkey.last_used_at}
                                            </small>
                                        )}
                                    </div>
